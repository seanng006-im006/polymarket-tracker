...
def compute_kyle(mid, cur_vol, cur_prob, question, win_label, hours):
    ...
    if vg>=threshold and abs(ps)>0.005:
        ...
        if win_label=="3h" and vg>=250_000:
            ...
            since_3h = (now_dt - timedelta(hours=3)).isoformat()
            try:
                tg_check = get_sb().table("spike_log").select("id")                    .eq("market_id", mid)                    .eq("win", "3h")                    .gte("ts", since_3h)                    .execute()
                if not tg_check.data:
